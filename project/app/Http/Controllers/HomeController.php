...
public function user()
    {   $searchtype='user';
         ...
         $myshops=ShopUser::where('user_id',Auth::user()->id)->get();
         $connectedshops=DB::table('shop_admins')->where('user_id',$uid)->get();
         $products=DB::table('products')->where('user_id',Auth::user()->id)->paginate(20);
        return view('gitbazar.profile',['myshops'=>$myshops ,'connectedshops'=>$connectedshops ,'user_profile'=>$user_profile ,'products'=>$products,'searchtype'=>$searchtype]);
