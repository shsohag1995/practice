          ...
          <td>{{$product->minimum_order}}</td>
           <td  class="actions">
              <a href="/shop/{{$shop->slug}}/product/{{$product->sp_counter_number}}/name/{{$product->slug}}" class="btn btn-info btn-sm ">View</a>
                <a href="/shop/{{$shop->id}}/product/{{$product->sp_counter_number}}/edit" class="btn btn-primary btn-sm ">Edit</a>
                @if($product->action=='show')
               <a href="/shop/{{$shop->id}}/product/{{$product->sp_counter_number}}/hide" class="btn btn-warning btn-sm ">Hide</a>
               @else
               <a href="/shop/{{$shop->id}}/product/{{$product->sp_counter_number}}/show" class="btn btn-success btn-sm ">Show</a>
               @endif
               @if($shop->type=='Public')
              <a href="/shop/{{$product->shop_id}}/product/{{$product->sp_counter_number}}/public/destroy" class="btn btn-danger btn-sm ">Delete</a>
              @else
              <a href="/shopmanage/{{$shop->id}}/product/{{$product->sp_counter_number}}/private/destroy" class="btn btn-danger btn-sm ">Delete</a>
              @endif
           </td>
        </tr>
         @php $i++; @endphp
