            ...
            <div class="container">
               ...
               <div class="row">
                  <div class="col-md-6 col-sm-6">
                    @if(Auth::check()&&($ok==1 ||$product->user_id==Auth::user()->id))
                     ...
                     <div class="single-product-image-inner">
                        ...
                        @foreach($images as $image)
                           <li  class=""><a href="#{{$i}}" aria-controls="{{$i}}" role="tab" data-toggle="tab"><img  src="{{Storage::url($image->image)}}" alt=""style="width: 30px;height:40px;float: left; "></a>
                            @if(Auth::check()&&($ok==1||$product->user_id==Auth::user()->id))
                            <a  href="/shop/{{$shop->id}}/product/{{$product->sp_counter_number}}/image/{{$image->created_at}}/public/destroy">
      ...
      @if($product->category)
       ...
       @endif
        ...
        <form class="form-group" action="/shop/private/product/wishlist/store" method="POST" enctype="multipart/form-data">
          ...
          <input class="form-group" required type="number" value="{{$product->minimum_order}}" name="quantity"><span>  {{$product->unit}} ({{$product->quantity}} {{$product->unit}} available)</span> 
